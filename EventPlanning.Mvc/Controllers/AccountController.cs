...
using EventPlanning.Mvc.ViewModel;
...
namespace EventPlanning.Mvc.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterModel model, Position position, Group group)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            ...
            var existingFullName = db.Set<FullName>().FirstOrDefault(m => 
                m.LastName == model.FullName.LastName &&
                m.FirstName == model.FullName.FirstName &&
                m.SureName == model.FullName.SureName);

            if (existingFullName == null)
            {
                existingFullName = db.FullNames.Add(model.FullName);
                db.SaveChanges();
            }
            ...
            model.Employee.GroupId = group.GroupId;
            model.Employee.PositionId = position.PositionId;
            model.Employee.AspNetUsersId = user.Id;
            model.Employee.FullNameId = existingFullName.FullNameId;
